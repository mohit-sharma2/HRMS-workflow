...
const GET_ALL_JOB_POSTINGS = gql`
  query GetAllJobPostings {
    getAllJobPostings(request: {}) {
      data {
        jobPostings {
          jobPostingId
          title
...
const UPDATE_CANDIDATE_STAGE = gql`
...
function JobDetailModal({ job, candidates, onClose, onUpdateCandidateStatus }: {
  ...
  onClose: () => void;
  onUpdateCandidateStatus: (id: string | number, status: string) => void;
}) {
...
export default function RecruitmentPage() {
  ...
  // Local State representing job and candidate lists (resilient fallback model)
  ...
  // Query definitions
  const { data: jobsData, loading: jobsLoading, refetch: refetchJobs } = useQuery(GET_ALL_JOB_POSTINGS, {
    variables: { request: {} },
    errorPolicy: 'all'
  });
  const { data: candidatesData, loading: candidatesLoading, refetch: refetchCandidates } = useQuery(GET_ALL_CANDIDATES, {
    variables: { request: {} },
    errorPolicy: 'all'
  });

  const [updateCandidateStage] = useMutation(UPDATE_CANDIDATE_STAGE);

  // Synchronize dynamic query content with interactive local states
  useEffect(() => {
    if (!jobsLoading && !candidatesLoading) {
      const rawCandidates = (candidatesData as any)?.getAllCandidates?.data?.candidates || [];
      const rawJobs = (jobsData as any)?.getAllJobPostings?.data?.jobPostings || [];

      let parsedCandidates: Candidate[] = [];
      if (rawCandidates.length === 0) {
        // Safe mock fallback when database seeding or GraphQL query details are offline
        parsedCandidates = CANDIDATES.map((c: any) => ({
          id: c.id,
          name: c.name,
          role: c.role,
          status: c.status,
        ...
        parsedCandidates = rawCandidates.map((c: any) => ({
          id: c.candidateId || '',
          name: c.fullName || '',
          role: c.appliedRole || '',
          status: c.status || 'New',
      ...
      let parsedJobs: Job[] = [];
      if (rawJobs.length === 0) {
        parsedJobs = JOB_POSTINGS.map((j: any) => {
          const jobCandidates = parsedCandidates.filter((c: any) => c.role === j.title);
          const shortlisted = jobCandidates.filter((c: any) => c.status === 'Shortlisted').length;
          const interviewing = jobCandidates.filter((c: any) => c.status === 'Interview Scheduled').length;
          return {
            id: j.id,
            title: j.title,
            ...
            shortlisted: j.shortlisted || shortlisted,
            interviewing: j.interviewing || interviewing,
            description: j.description || ''
          };
        });
      } else {
        parsedJobs = rawJobs.map((j: any) => {
          const jobCandidates = parsedCandidates.filter((c: any) => c.role === j.title);
          const shortlisted = jobCandidates.filter((c: any) => c.status === 'Shortlisted').length;
          const interviewing = jobCandidates.filter((c: any) => c.status === 'Interview Scheduled').length;
          return {
            ...
            applicants: j.totalCandidates || jobCandidates.length,
            shortlisted,
            interviewing,
            description: j.description || ''
          };
  ...
  // Aggregate stats
  const totalApplicants = jobs.reduce((acc: number, j) => acc + j.applicants, 0);
  const totalShortlisted = jobs.reduce((acc: number, j) => acc + j.shortlisted, 0);
  const totalInterviewing = jobs.reduce((acc: number, j) => acc + j.interviewing, 0);
  const activeJobs = jobs.filter(j => j.status === 'Active').length;

  const handleCandidateStatusChange = async (id: string | number, status: string) => {
    ...
    try {
      ...
      console.warn("GraphQL backend offline, synced local candidate status only", err);
    }
  };
  ...
  // Pre-load layout blocker during initial query initialization
  if ((jobsLoading || candidatesLoading) && jobs.length === 0) {
    return (
      <div className="flex h-[400px] items-center justify-center bg-white rounded-xl border border-gray-100 shadow-sm animate-pulse">
        <div className="text-teal-600 font-medium text-sm">Loading recruitment pipeline details...</div>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <button 
          ...
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-teal-600 text-white text-sm font-medium hover:bg-teal-700 transition-colors shadow-sm self-start sm:self-auto"
        >
          <PlusIcon />
          New Job Posting
        </button>
      ...
      {/* Add Job Modal */}
      {showAddJobModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={() => setShowAddJobModal(false)}>
          <div className="bg-white rounded-2xl w-full max-w-md shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="font-bold text-gray-900">New Job Posting</h3>
            ...
            <form onSubmit={handleAddJob} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:border-teal-500"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:border-teal-500"
                  >
                    {['Engineering', 'Product', 'Design', 'HR', 'Marketing', 'Sales'].map(d => <option key={d}>{d}</option>)}
                  </select>
                </div>
                <div>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase mb-1">Salary Range</label>
                <input
                  type="text"
                  required
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:outline-none focus:border-teal-500 resize-none"
                />
              </div>
