...
export default function RecognitionPage() {
  ...
  const [comments, setComments] = useState<Record<number, string[]>>({
    1: ['So well deserved!', 'Great job Alex!'],
    2: ['A true leader indeed.'],
  });
            ...
            <div key={rec.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {/* Footer */}
                  <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-50 bg-white">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => {
                          if (showComments.includes(rec.id)) {
                            setShowComments(showComments.filter(cid => cid !== rec.id));
                          } else {
                            setShowComments([...showComments, rec.id]);
                          }
                        }}
                        className="flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg bg-gray-50 text-gray-500 hover:bg-gray-100 font-medium transition-colors"
                      >
                        Comments ({(comments[rec.id] || []).length})
                      </button>
                      <button
                        ...
                        className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg transition-colors font-medium
                          ${isLiked ? 'bg-red-50 text-red-500 font-semibold' : 'bg-gray-50 text-gray-500 hover:bg-red-50 hover:text-red-500'}`}
                      >
                  ...
                  {/* Comments list/add */}
                  {showComments.includes(rec.id) && (
                    <div className="mt-3 pt-3 border-t border-gray-50 bg-gray-50/50 rounded-xl p-3">
                      {(comments[rec.id] || []).length > 0 && (
                        <div className="space-y-1.5 mb-3">
                          {(comments[rec.id] || []).map((cmt, idx) => (
                            <div key={idx} className="text-xs bg-white rounded-lg p-2 border border-gray-100 shadow-xs">
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          onChange={e => setCommentInput({ ...commentInput, [rec.id]: e.target.value })}
                          className="flex-1 text-xs rounded-lg border border-gray-200 px-3 py-1.5 focus:outline-none focus:ring-1 focus:ring-teal-500 bg-white"
                          onKeyDown={e => e.key === 'Enter' && handleAddComment(rec.id)}
                        />
                        <button
