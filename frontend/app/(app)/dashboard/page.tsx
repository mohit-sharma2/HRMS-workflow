...
//   if (isLoading || !user) return <div className="flex h-screen items-center justify-center"><div className="animate-spin w-8 h-8 rounded-full border-4 border-teal-600 border-t-transparent" /></div>;

//   const todayAttendance = ATTENDANCE_DATA[ATTENDANCE_DATA.length - 1];
//   const presentDays = ATTENDANCE_DATA.filter(a => a.status === 'Present').length;
...
//         {/* Clock In/Out — only for Employee */}
//         {user.role === 'Employee' && (
//           <button
//             onClick={() => setClockedIn(!clockedIn)}
//             className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-medium text-sm transition-all shadow-sm
...
//             <Clock size={16} />
//             {clockedIn ? 'Clock Out' : 'Clock In'}
//             {!clockedIn && <span className="text-teal-200 text-xs">{time.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</span>}
//           </button>
//         )}
//       </div>
...
//           <div className="flex items-center justify-between mb-4">
//             <h2 className="font-semibold text-gray-900">Recent Attendance</h2>
//             <a href="/attendance" className="text-xs text-teal-600 hover:underline flex items-center gap-1">View all <ArrowRight size={12} /></a>
//           </div>
//           <div className="space-y-2">
//             {ATTENDANCE_DATA.slice(-5).reverse().map((a) => (
//               <div key={a.date} className="flex items-center justify-between py-1.5 border-b border-gray-50 last:border-0">
//                 <span className="text-sm text-gray-600">{new Date(a.date).toLocaleDateString('en-IN', { weekday: 'short', day: 'numeric', month: 'short' })}</span>
//                 <div className="flex items-center gap-3">
//                   <span className="text-xs text-gray-400">{a.clockIn || '--'} → {a.clockOut || '--'}</span>
//                   <span className={`text-xs px-2 py-0.5 rounded-full font-medium
//                     ${a.status === 'Present' ? 'bg-green-100 text-green-700' :
...
//           <div className="flex items-center justify-between mb-4">
//             <h2 className="font-semibold text-gray-900">Announcements</h2>
//             <a href="/announcements" className="text-xs text-teal-600 hover:underline flex items-center gap-1">View all <ArrowRight size={12} /></a>
//           </div>
//           <div className="space-y-3">
//             {ANNOUNCEMENTS.slice(0,3).map((a) => (
//               <div key={a.id} className="flex gap-3 pb-3 border-b border-gray-50 last:border-0">
...
//                 <div>
//                   <p className="text-sm font-medium text-gray-800">{a.title}</p>
//                   <p className="text-xs text-gray-400 mt-0.5">{a.category} · {new Date(a.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</p>
//                 </div>
//               </div>
//             ))}
//           </div>
...
//             <div className="flex items-center justify-between mb-4">
//               <h2 className="font-semibold text-gray-900">My Goals</h2>
//               <a href="/performance" className="text-xs text-teal-600 hover:underline flex items-center gap-1">View all <ArrowRight size={12} /></a>
//             </div>
//             <div className="space-y-3">
//               {GOALS.map((g) => (
//                 <div key={g.id}>
...
//             <div className="flex items-center justify-between mb-4">
//               <h2 className="font-semibold text-gray-900">Leave Balance</h2>
//               <a href="/leave" className="text-xs text-teal-600 hover:underline flex items-center gap-1">Apply <ArrowRight size={12} /></a>
//             </div>
//             <div className="space-y-3">
//               {LEAVE_BALANCE.map((l) => (
//                 <div key={l.type} className="flex items-center justify-between">
